...
local function imguicallback()
  openGUI, shouldDrawHUD = ImGui.Begin(echo.icon.radar .. ' radar###EntropyRadar'..classID..name, openGUI)
        ...
        for x = 1, sortcount, 1 do
          ...
          if nearspawn.ID() ~= nil then
            ...
            if ImGui.TreeNode('##'..x) then
              ...
              ImGui.Text('::')
              ImGui.SameLine()
              ImGui.Text(round(nearspawn.Height(), 2))
          ...
          ImGui.TableNextColumn()
            if xtspawn.ID() == 0 then
              ...
              if mq.TLO.Target.ID() == xtspawn.ID() then
                ImGui.SameLine()
                ImGui.Text(echo.icon.arrowleft)
              end
            end
          ImGui.TableNextColumn()
            if xtspawn.ID() ~= 0 then
